...
function KebabMenu({ onArchive, onDelete }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-8 h-8 flex items-center justify-center rounded-lg text-[#8B8FA8] hover:text-[#E8E8F0] hover:bg-[#2A2D38] transition-colors cursor-pointer"
        title="Options"
      >
        ⋮
      </button>
      {open && (
        <div className="absolute right-0 top-9 w-40 bg-[#1E2028] border border-[#2A2D38] rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] z-20 overflow-hidden">
          <button
            onClick={() => { onArchive(); setOpen(false) }}
            className="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-[#B0B3C6] hover:bg-[#252830] hover:text-[#E8E8F0] transition-colors cursor-pointer"
          >
      ...
      <div className="mt-8">
        {allDone ? (
          <div className="text-center space-y-1">
            ...
            <p className="text-[#8B8FA8] text-xs">You are extraordinary. Keep going.</p>
          </div>
        ) : (
          <div className="flex items-center gap-3">
            <div className="text-left min-w-[32px]">
              <p className="text-[#E8E8F0] text-sm font-bold tabular-nums">{days}</p>
...
function TimeUnit({ value, label, highlight }) {
  return (
    <div className={`
      ...
      ${highlight
        ? 'bg-gradient-to-b from-[#C17A47] to-[#A5622F] shadow-[0_4px_20px_rgba(193,122,71,0.4)]'
        : 'bg-[#1E2028] border border-[#2A2D38]'}
    `}>
      {highlight && (
      ...
      <span className={`
        ...
        ${highlight ? 'text-white' : 'text-[#E8E8F0]'}
      `}>{value}</span>
      <span className={`
        text-[10px] mt-1.5 uppercase tracking-widest font-semibold
        ${highlight ? 'text-orange-100' : 'text-[#8B8FA8]'}
      `}>{label}</span>
    </div>
  )
}
...
export default function CounterCard({ timer, onReset, onArchive, onDelete }) {
  ...
  return (
    <div className="bg-[#16181F] border border-[#2A2D38] rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.3)] hover:shadow-[0_8px_40px_rgba(0,0,0,0.4)] hover:border-[#333644] transition-all duration-300">

      {/* Header */}
      <div className="relative px-5 pt-5 pb-4 flex items-start justify-between">
        {/* Ambient glow */}
        <div className="absolute top-0 right-0 w-40 h-40 bg-[#C17A47]/6 rounded-full blur-3xl pointer-events-none -translate-y-1/2 translate-x-1/4" />

        <div className="relative">
          <div className="flex items-center gap-2 mb-0.5">
            {/* Live indicator */}
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#C17A47] opacity-50" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-[#C17A47]" />
            </span>
            <h3 className="text-[#E8E8F0] font-bold text-base">{timer.addiction_name}</h3>
          </div>
          <p className="text-[#8B8FA8] text-xs pl-4">
            Since {new Date(timer.started_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
          </p>
          {latestMilestoneReached && (
            <span className="mt-2.5 ml-4 inline-flex items-center gap-1 text-[11px] font-bold text-[#C17A47] bg-[#C17A47]/10 border border-[#C17A47]/20 rounded-full px-2.5 py-1">
      ...
      {/* Divider */}
      <div className="mx-5 h-px bg-gradient-to-r from-transparent via-[#2A2D38] to-transparent" />
      ...
      <div className="px-5 pt-5 pb-1">
        <div className="flex gap-2 justify-center items-end">
          ...
          <div className="flex items-center mb-[18px] text-[#2A2D38] font-bold text-lg select-none">:</div>
