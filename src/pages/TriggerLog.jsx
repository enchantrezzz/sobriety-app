...
export default function TriggerLog() {
  ...
  return (
    ...
    >
      ...
      <motion.p variants={itemVariants} className="text-[#8B8FA8] text-sm mb-6">
      ...
      {!loading && triggers.length === 0 && (
        <motion.div variants={itemVariants} className="text-center py-16 text-[#8B8FA8]">
      ...
      <div className="space-y-3">
        {triggers.map(t => (
          <motion.div
            ...
            key={t.id}
            className="bg-[#16181F] border border-[#2A2D38] rounded-2xl p-4 hover:border-[#333644] transition-colors"
          >
      ...
      <AnimatePresence>
        {showForm && (
          <div className="fixed inset-0 flex items-center justify-center z-50 px-4 overflow-y-auto py-8">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.form
              ...
              className="bg-[#16181F] border border-[#2A2D38] rounded-2xl p-6 max-w-sm w-full shadow-[0_24px_80px_rgba(0,0,0,0.6)] space-y-4 relative z-10"
            >
